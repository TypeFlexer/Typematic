...
protected:
    ...
    // Only subclasses should call this
    TaintedConstraintVariable(ConstraintVariableKind K, std::string T, std::string N,
                       std::string TN)
            : Kind(K), OriginalType(T), Name(N), OriginalTypeWithName(TN),
              HasEqArgumentConstraints(false), ValidBoundsKey(false),
              IsForDecl(false) {}

    TaintedConstraintVariable(ConstraintVariableKind K, QualType QT, std::string N)
            : TaintedConstraintVariable(K, qtyToStr(QT), N,
                                 qtyToStr(QT, N == RETVAR ? "" : N)) {}
...
public:
    ...
    virtual bool solutionEqualTo(Constraints &, const TaintedConstraintVariable *,
                                 bool ComparePtyp = true) const = 0;
    ...
    // might solve to WILD, and unchecked variables might solve to checked. Use
    // isSolutionChecked if you want these final solved types.
    virtual bool isOriginallyTainted() const = 0;

    // Returns true if any of the constraint variables 'within' this instance
    // have a binding in E other than top. E should be the EnvironmentMap that
    // results from running unification on the set of constraints and the
    // environment.
    virtual bool isSolutionTainted(const TaintedEnvironmentMap &E) const = 0;
    ...
    // Return true if all atoms (pointer levels) in this variable are tainted.
    // This is in contrast to isSolutionChecked which will return true if any atom
    // is checked.
    virtual bool isSolutionFullyTainted(const TaintedEnvironmentMap &E) const = 0;
    ...
    // with this constraint variable. Used with redeclarations, especially of
    // functions declared in multiple files.
    virtual void mergeDeclaration(TaintedConstraintVariable *, ProgramInfo &,
                                  std::string &ReasonFailed) = 0;
    ...
    // fresh atoms allows the new variable to solve to different types than the
    // original.
    virtual TaintedConstraintVariable *getCopy(ReasonLoc &Rsn, Constraints &CS) = 0;
...
void TaintedconstrainConsVarGeq(const std::set<TaintedConstraintVariable *> &LHS,
                         const std::set<TaintedConstraintVariable *> &RHS,
                         Constraints &CS, const ReasonLoc &Rsn,
                         ConsAction CA, bool DoEqType, ProgramInfo *Info,
                         bool HandleBoundsKey = true);
...
bool isAValidTPVConstraint(const TaintedConstraintVariable *C);
...
public:
    ...
    //       This causes problems if the variable is later used as a deeper
    //       pointer type. See correctcomputation/checkedc-clang#673.
    static TaintedPointerVariableConstraint *
    getWildPVConstraint(Constraints &CS, const ReasonLoc &Rsn);

    // Get constraint variables representing values that are not pointers. If a
    // meaningful name can be assigned to the value, the second method should be
    // used to get higher quality root-cause and debugging output.
    static TaintedPointerVariableConstraint *getNonPtrPVConstraint(Constraints &CS);
    static TaintedPointerVariableConstraint *getNamedNonPtrPVConstraint(StringRef Name,
                                                                 Constraints &CS);
    ...
    // by the original constraint variable.
    static TaintedPointerVariableConstraint *
    addAtomPVConstraint(TaintedPointerVariableConstraint *PVC, ConstAtom *PtrTyp,
                        ReasonLoc &Rsn, Constraints &CS);
    ...
    // original.
    static TaintedPointerVariableConstraint *
    derefPVConstraint(TaintedPointerVariableConstraint *PVC);

private:
    std::string BaseType;
    CAtoms Vars;
    std::vector<ConstAtom *> SrcVars;
    TaintedFunctionVariableConstraint *FV;
    std::map<uint32_t, std::set<Qualification>> QualMap;
    enum OriginalArrType { O_Pointer, O_SizedArray, O_UnSizedArray };
    // Map from pointer idx to original type and size.
    // If the original variable U was:
    ...
    // this set contains the constraint variable of
    // the values used as arguments.
    std::set<TaintedConstraintVariable *> ArgumentConstraints;
    // Get solution for the atom of a pointer.
    const ConstAtom *getSolution(const Atom *A, const TaintedEnvironmentMap &E) const;

    // Construct a copy of this variable, reusing all VarAtoms. To instead obtains
    // a copy of the constraint variable which contains fresh VarAtoms (allowing
    // the new variable to solve to different type than the original), instead use
    // the method getCopy.
    TaintedPointerVariableConstraint(TaintedPointerVariableConstraint *Ot);
    TaintedPointerVariableConstraint *Parent;
    // String representing declared bounds expression.
    std::string BoundsAnnotationStr;
    ...
    bool IsTypedef = false;
    TaintedConstraintVariable *TypedefVar;
    std::string TypedefString;
    // Does the type internally contain a typedef, and if so: at what level and
    // what is it's name?
    struct InternalTypedefInfo TypedefLevelInfo;
    ...
    // other fields are initialized to default values. This is used to construct
    // variables for non-pointer expressions.
    TaintedPointerVariableConstraint(std::string Name) :
            TaintedConstraintVariable(PointerVariable, "", Name, ""), FV(nullptr),
            SrcHasItype(false), PartOfFuncPrototype(false), Parent(nullptr),
            SourceGenericIndex(-1), InferredGenericIndex(-1),
            IsZeroWidthArray(false), IsTypedef(false),
            TypedefLevelInfo({}), IsVoidPtr(false) {}
...
public:
    ...
    bool isTypedef(void) const;
    const TaintedConstraintVariable *getTypedefVar() const;
    void setTypedef(TaintedConstraintVariable *TDVar, std::string S);
    ...
    // kept that way during solving, (2) nothing that was originally checked
    // should be modified during rewriting.
    bool isOriginallyTainted() const override {
      return llvm::any_of(Vars, [](Atom *A) { return isa<ConstAtom>(A); });
    }

    bool isSolutionTainted(const TaintedEnvironmentMap &E) const override;
    bool isSolutionFullyTainted(const TaintedEnvironmentMap &E) const override;

    bool isVoidPtr() const { return IsVoidPtr; }

    bool solutionEqualTo(Constraints &CS, const TaintedConstraintVariable *CV,
                         bool ComparePtyp = true) const override;
    ...
    // constructor below with type, type source info, and name instantiated with
    // information in D.
    TaintedPointerVariableConstraint(clang::DeclaratorDecl *D, ProgramInfo &I,
                              const clang::ASTContext &C);
    TaintedPointerVariableConstraint(clang::TypedefDecl *D, ProgramInfo &I,
                              const clang::ASTContext &C);
    TaintedPointerVariableConstraint(clang::Expr *E, ProgramInfo &I,
                              const clang::ASTContext &C);
    ...
    //      preserving the exact syntax used to write types that aren't rewritten
    //      by 3C. If this is null, then the type will be reconstructed from QT.
    TaintedPointerVariableConstraint(const clang::QualType &QT, clang::DeclaratorDecl *D,
                              std::string N, ProgramInfo &I,
                              const clang::ASTContext &C,
                              std::string *InFunc = nullptr,
                              int ForceGenericIndex = -1,
    ...
    // Include new ConstAtoms, supplemental info, and merge function pointers
    void mergeDeclaration(TaintedConstraintVariable *From, ProgramInfo &I,
                          std::string &ReasonFailed) override;

    static bool classof(const TaintedConstraintVariable *S) {
      return S->getKind() == PointerVariable;
    }
    ...
    TaintedFunctionVariableConstraint *getFV() const { return FV; }
    ...
    bool isNtConstantArr(const TaintedEnvironmentMap &E) const;
    bool isConstantArr() const;
    unsigned long getConstantArrSize() const;
    ...
    bool isPartOfFunctionPrototype() const { return PartOfFuncPrototype; }
    // Add the provided constraint variable as an argument constraint.
    bool addArgumentConstraint(TaintedConstraintVariable *DstCons,
                               ReasonLoc &Rsn, ProgramInfo &Info);
    // Get the set of constraint variables corresponding to the arguments.
    const std::set<TaintedConstraintVariable *> &getArgumentConstraints() const;

    TaintedPointerVariableConstraint *getCopy(ReasonLoc &Rsn, Constraints &CS) override;
    ...
    ~TaintedPointerVariableConstraint() override{};
...
public:
  ...
  bool hasTaintedSolution(Constraints &CS) const;
...
private:
    friend class TaintedFunctionVariableConstraint;
    TPVConstraint *InternalConstraint;
    TPVConstraint *ExternalConstraint;
...
public:
    TFVComponentVariable()
            : InternalConstraint(nullptr), ExternalConstraint(nullptr),
              SourceDeclaration("") {}

    TFVComponentVariable(TFVComponentVariable *Ot, ReasonLoc &Rsn, Constraints &CS);

    TFVComponentVariable(const clang::QualType &QT, const clang::QualType &ITypeT,
                        clang::DeclaratorDecl *D, std::string N, ProgramInfo &I,
                        const clang::ASTContext &C, std::string *InFunc,
                        bool PotentialGeneric, bool HasItype, bool IsTaintedfunction);

    void mergeDeclaration(TFVComponentVariable *From, ProgramInfo &I,
                          std::string &ReasonFailed);
    std::string mkItypeStr(Constraints &CS, bool ForItypeBase = false) const;
    std::string mkTypeStr(Constraints &CS, bool EmitName,
                          std::string UseName = "",
    ...
    TPVConstraint *getInternal() const { return InternalConstraint; }
    TPVConstraint *getExternal() const { return ExternalConstraint; }
...
// Constraints on a function type. Also contains a 'name' parameter for
// when a re-write of a function pointer is needed.
class TaintedFunctionVariableConstraint : public TaintedConstraintVariable {
private:
    TaintedFunctionVariableConstraint(TaintedFunctionVariableConstraint *Ot);

    // N constraints on the return value of the function.
    TFVComponentVariable ReturnVar;
    // A vector of K sets of N constraints on the parameter values, for
    // K parameters accepted by the function.
    std::vector<TFVComponentVariable> ParamVars;
    ...
    bool Hasbody;
    bool IsStatic;
    TaintedFunctionVariableConstraint *Parent;
    // Flag to indicate whether this is a function pointer or not.
    bool IsFunctionPtr;
    ...
    void equateFVConstraintVars(TaintedConstraintVariable *CV, ProgramInfo &Info,
                                ReasonLoc &Rsn) const;

public:
    TaintedFunctionVariableConstraint(clang::DeclaratorDecl *D, ProgramInfo &I,
                               const clang::ASTContext &C);
    TaintedFunctionVariableConstraint(clang::TypedefDecl *D, ProgramInfo &I,
                               const clang::ASTContext &C);

    TaintedFunctionVariableConstraint(const clang::QualType Ty,
                               clang::DeclaratorDecl *D, std::string N,
                               ProgramInfo &I, const clang::ASTContext &C,
                               TypeSourceInfo *TSI = nullptr);

    TPVConstraint *getExternalReturn() const {
      return ReturnVar.ExternalConstraint;
    }

    TPVConstraint *getInternalReturn() const {
      return ReturnVar.InternalConstraint;
    }

    const TFVComponentVariable *getCombineReturn() const { return &ReturnVar; }
    ...
    // Merge return value and all params
    void mergeDeclaration(TaintedConstraintVariable *FromCV, ProgramInfo &I,
                          std::string &ReasonFailed) override;

    TPVConstraint *getExternalParam(unsigned I) const {
      assert(I < ParamVars.size());
      return ParamVars.at(I).ExternalConstraint;
    }

    TPVConstraint *getInternalParam(unsigned I) const {
      assert(I < ParamVars.size());
      return ParamVars.at(I).InternalConstraint;
    }

    const TFVComponentVariable *getCombineParam(unsigned I) const {
      assert(I < ParamVars.size());
      return &ParamVars.at(I);
    }
    ...
    TaintedFunctionVariableConstraint *getCopy(ReasonLoc &Rsn, Constraints &CS) override;
    ...
    ~TaintedFunctionVariableConstraint() override {}
};

typedef FunctionVariableConstraint FVConstraint;
typedef TaintedFunctionVariableConstraint TFVConstraint;
