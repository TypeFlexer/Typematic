...
checkConditionalObjectPointersCompatibility(Sema &S, ExprResult &LHS,
  ...
  const PointerType *lhpt = LHSTy->castAs<PointerType>();
...
static bool checkUncheckedPointerIntegerMismatch(Sema &S, ExprResult &Int,
  ...
  if (ptrTy->isChecked() && !S.getLangOpts()._3C) {
     return false;
  }
 ...
 */
  ...
  if(!isTaintedAssignmentValid(lhkind, rhkind, FDecl))
  {
  ...
  if (!S.Context.pointeeTypesAreAssignable(ltrans, rtrans)) {
     ...
     if (LHSType->isOrContainsCheckedOrTaintedType() ||
