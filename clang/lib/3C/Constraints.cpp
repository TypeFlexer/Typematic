        ...
        for idx, constraint in enumerate(all_constraints, 1):
            ...
            if match:
                ...
                if 'Pointer within a Tainted Structure' in reason:
                    ...
                    # remove the node between TPTR and struct
                    ...
                    if ('TPTR', special_tptr) not in edges:
                        ...
                        added_new = True
                        print(f"    Added special edge: 'TPTR' -> '{special_tptr}'\n")
...
void Constraints::printISTM(raw_ostream &O) const {
    std::error_code EC;
    llvm::raw_fd_ostream tempFile("constraints_temp.txt", EC, llvm::sys::fs::OF_None);

    // Print constraints to both standard output and the temporary file
    O << "Choke Analysis: \n";
    tempFile << "Choke Analysis: \n";
