...
void Constraints::printISTM(raw_ostream &O, EnvironmentMap Environment, std::string python_script) const {
    std::error_code EC;

    // Remove existing files if they exist
    llvm::sys::fs::remove("constraints_temp.txt");
    llvm::sys::fs::remove("constraint_env.txt");

    // Create the files
    llvm::raw_fd_ostream constraintsFile("constraints_temp.txt", EC, llvm::sys::fs::OF_None);
    if (EC) {
        O << "Error creating constraints file: " << EC.message() << "\n";
        return;
    }

    llvm::raw_fd_ostream envFile("constraint_env.txt", EC, llvm::sys::fs::OF_None);
    if (EC) {
        O << "Error creating environment file: " << EC.message() << "\n";
        return;
    }

    // Print constraints to both standard output and the constraints file
    O << "Choke Analysis: \n";
    constraintsFile << "Choke Analysis: \n";

    for (const auto &C : TheConstraints) {
        C->print(O);
        O << "\n";

        C->print(constraintsFile);
        constraintsFile << "\n";
    }
    ...
    // Print the environment to both standard output and the environment file
    O << "ENVIRONMENT:\n";
    envFile << "ENVIRONMENT:\n";
    ...
    for (const auto &V : Environment) {
        ...
        // Print to environment file
        V.first->print(envFile);
        envFile << " = [";
        envFile << "Checked=";
        V.second.first->print(envFile);
        envFile << "[ " << CheckedGraphInfluence << " ]";
        envFile << ", PtrType=";
        V.second.second->print(envFile);
        envFile << "[ " << PtrTyGraphInfluence << " ]";
        envFile << "]";
        envFile << "\n";
    }
    ...
    // Append the script filename
    scriptPath += "/taint_analysis.py";

    // Read the Python script into a string
    std::ifstream scriptFile(scriptPath);
    if (!scriptFile) {
        O << "Error: Could not open Python script file at " << scriptPath << "\n";
        Py_Finalize();
        return;
    }

    std::string pythonCode((std::istreambuf_iterator<char>(scriptFile)),
    ...
    if (result != 0) {
        O << "Error: Python script execution failed.\n";
        // Optionally, you can fetch and print the Python error here
        if (PyErr_Occurred()) {
            PyErr_Print();
        }
    }
