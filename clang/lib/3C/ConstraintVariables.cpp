...
std::string TaintedConstraintVariable::getRewritableOriginalTy() const {
  std::string OrigTyString = getOriginalTy();
  std::string SpaceStr = " ";
  std::string AsterixStr = "*";
  // If the type does not end with " " or *
...
std::string TaintedConstraintVariable::getOriginalTypeWithName() const {
  if (Name == RETVAR)
    return getRewritableOriginalTy();
  return OriginalTypeWithName;
}
...
TaintedPointerVariableConstraint *TaintedPointerVariableConstraint::getWildPVConstraint(
        Constraints &CS, const ReasonLoc &Rsn) {
  auto *WildPVC = new TaintedPointerVariableConstraint("wildvar");
  VarAtom *VA = CS.createFreshTGEQ("wildvar", VarAtom::V_Other, CS.getWild(),
                                  Rsn);
  WildPVC->Vars.push_back(VA);
  WildPVC->SrcVars.push_back(CS.getWild());
...
TaintedPointerVariableConstraint *
TaintedPointerVariableConstraint::getNonPtrPVConstraint(Constraints &CS) {
  static TaintedPointerVariableConstraint *GlobalNonPtrPV = nullptr;
  if (GlobalNonPtrPV == nullptr)
    GlobalNonPtrPV = new TaintedPointerVariableConstraint("basevar");
  return GlobalNonPtrPV;
}
...
TaintedPointerVariableConstraint *
TaintedPointerVariableConstraint::getNamedNonPtrPVConstraint(StringRef Name,
                                                      Constraints &CS) {
  return new TaintedPointerVariableConstraint(std::string(Name));
}
...
TaintedPointerVariableConstraint *
TaintedPointerVariableConstraint::derefPVConstraint(TaintedPointerVariableConstraint *PVC) {
  // Make a copy of the PVConstraint using the same VarAtoms
  auto *Copy = new TaintedPointerVariableConstraint(PVC);
...
TaintedPointerVariableConstraint::TaintedPointerVariableConstraint(
        TaintedPointerVariableConstraint *Ot)
        : TaintedConstraintVariable(TaintedConstraintVariable::PointerVariable, Ot->OriginalType,
                             Ot->Name, Ot->OriginalTypeWithName),
          BaseType(Ot->BaseType), Vars(Ot->Vars), SrcVars(Ot->SrcVars), FV(Ot->FV),
          QualMap(Ot->QualMap), ArrSizes(Ot->ArrSizes), ArrSizeStrs(Ot->ArrSizeStrs),
          SrcHasItype(Ot->SrcHasItype), ItypeStr(Ot->ItypeStr),
...
TaintedPointerVariableConstraint::TaintedPointerVariableConstraint(DeclaratorDecl *D,
                                                     ProgramInfo &I,
                                                     const ASTContext &C)
        : TaintedPointerVariableConstraint(D->getType(), D, std::string(D->getName()), I,
                                    C, nullptr, -1, false, false,
                                    D->getTypeSourceInfo()) {}
...
TaintedPointerVariableConstraint::TaintedPointerVariableConstraint(TypedefDecl *D,
                                                     ProgramInfo &I,
                                                     const ASTContext &C)
        : TaintedPointerVariableConstraint(D->getUnderlyingType(), nullptr,
                                    D->getNameAsString(), I, C, nullptr, -1, false,
                                    false, D->getTypeSourceInfo()) {}
...
TaintedPointerVariableConstraint::TaintedPointerVariableConstraint(Expr *E, ProgramInfo &I,
                                                     const ASTContext &C)
        : TaintedPointerVariableConstraint(E->getType(), nullptr, E->getStmtClassName(), I,
                                    C, nullptr) {}
...
TaintedPointerVariableConstraint::TaintedPointerVariableConstraint(
        ...
        const ASTContext &C, std::string *InFunc, int ForceGenericIndex,
        bool PotentialGeneric,
        bool VarAtomForTainted, TypeSourceInfo *TSInfo, const QualType &ITypeT)
        : TaintedConstraintVariable(TaintedConstraintVariable::PointerVariable, QT, N),
          FV(nullptr), SrcHasItype(false), PartOfFuncPrototype(InFunc != nullptr),
          Parent(nullptr) {
  PersistentSourceLoc PSL = PersistentSourceLoc::mkPSL(D, C);
  QualType QTy = QT;
  ...
  while (Ty->isPointerType() || Ty->isArrayType()) {
    ...
    if (isVarArgType(TyName)) {
      // Variable number of arguments. Make it WILD.
      auto Rsn = ReasonLoc("Variable number of arguments.", PSL);
      VarAtom *WildVA = CS.createFreshTGEQ(Npre + N, VK, CS.getWild(), Rsn);
      Vars.push_back(WildVA);
      SrcVars.push_back(CS.getWild());
      VarCreated = true;
      break;
    ...
    if (!VarCreated) {
      ...
      auto Rsn = ReasonLoc("0-sized Array", PSL);
      if (ArrSizes[TypeIdx].first == O_SizedArray) {
        CS.addTaintedConstraint(CS.createTaintedGeq(CS.getTaintedArr(), VA, Rsn, false));
        // A constant array declared with size 0 cannot be _Nt_checked. Checked
        // C requires that _Nt_checked arrays are not empty since the declared
        // size of the array includes the null terminator.
        if (ArrSizes[TypeIdx].second == 0)
          CS.addTaintedConstraint(CS.createTaintedGeq(VA, CS.getTaintedArr(), Rsn, false));
      }
    }
  ...
  if (Ty->isFunctionType())
    ...
    // where tn is the typedef'ed type name.
    // There is possibly something more elegant to do in the code here.
    FV = new TFVConstraint(QTy, IsDeclTy ? D : nullptr, IsTypedef ? "" : N, I, C,
                          TSInfo);
  ...
  if (IsWild) {
    ...
    for (const auto &V : Vars)
      if (VarAtom *VA = dyn_cast<VarAtom>(V))
        CS.addTaintedConstraint(CS.createTaintedGeq(VA, CS.getWild(), ReasonLoc(Rsn,PSL)));
  }
...
std::string TaintedPointerVariableConstraint::tryExtractBaseType(
        MultiDeclMemberDecl *D, TypeSourceInfo *TSI, QualType QT, const Type *Ty,
        const ASTContext &C) {
  // Implicit parameters declarations from typedef function declarations will
  // still have valid and non-empty source ranges, but implicit declarations
...
std::string TaintedPointerVariableConstraint::extractBaseType(
        MultiDeclMemberDecl *D, TypeSourceInfo *TSI, QualType QT, const Type *Ty,
        const ASTContext &C, ProgramInfo &Info) {
  if (llvm::Optional<std::string> TypeStrOverride =
          Info.TheMultiDeclsInfo.getTypeStrOverride(Ty, C))
...
void TaintedPointerVariableConstraint::print(raw_ostream &O) const {
  O << "{ ";
  for (const auto &I : Vars) {
    I->print(O);
    O << " ";
...
bool TaintedPointerVariableConstraint::emitArraySize(
        std::stack<std::string> &ConstSizeArrs, uint32_t TypeIdx,
        Atom::AtomKind Kind) const {
  auto I = ArrSizes.find(TypeIdx);
  assert(I != ArrSizes.end());
  ...
  if (Oat == O_SizedArray) {
    std::ostringstream SizeStr;
    if (Kind != Atom::A_Wild)
      SizeStr << (Kind == Atom::A_NTArr ? " __TNt__" : " __T__");
    if (ArrSizeStrs.find(TypeIdx) != ArrSizeStrs.end()) {
      std::string SrcSizeStr = ArrSizeStrs.find(TypeIdx)->second;
      assert(!SrcSizeStr.empty());
      // In some weird edge cases the size of the array is defined by a macro
...
void TaintedPointerVariableConstraint::addArrayAnnotations(
        std::stack<std::string> &ConstArrs,
        std::deque<std::string> &EndStrs) const {
  while (!ConstArrs.empty()) {
    auto NextStr = ConstArrs.top();
...
bool TaintedPointerVariableConstraint::isTypedef(void) const { return IsTypedef; }
...
void TaintedPointerVariableConstraint::setTypedef(TaintedConstraintVariable *TDVar,
                                           std::string S) {
  IsTypedef = true;
  TypedefVar = TDVar;
  TypedefString = S;
...
const TaintedConstraintVariable *TaintedPointerVariableConstraint::getTypedefVar() const {
  assert(isTypedef());
  return TypedefVar;
}
...
std::string TaintedPointerVariableConstraint::gatherQualStrings(void) const {
  std::ostringstream S;
  getQualString(0, S);
  return S.str();
}
...
bool TPVConstraint::addArgumentConstraint(TaintedConstraintVariable *DstCons,
                                         ReasonLoc &Rsn,
                                         ProgramInfo &Info) {
  if (this->Parent == nullptr) {
    bool RetVal = false;
    if (isPartOfFunctionPrototype()) {
      RetVal = ArgumentConstraints.insert(DstCons).second;
      if (RetVal && this->HasEqArgumentConstraints) {
        TaintedconstrainConsVarGeq(DstCons, this, Info.getConstraints(), Rsn,
                            Same_to_Same, true, &Info);
      }
    }
    return RetVal;
...
const TVarSet &TPVConstraint::getArgumentConstraints() const {
  return ArgumentConstraints;
}
...
TaintedFunctionVariableConstraint::TaintedFunctionVariableConstraint(TFVConstraint *Ot)
        : TaintedConstraintVariable(TaintedConstraintVariable::FunctionVariable, Ot->OriginalType,
                             Ot->getName(), Ot->OriginalTypeWithName),
          ReturnVar(Ot->ReturnVar), ParamVars(Ot->ParamVars), FileName(Ot->FileName),
          Hasproto(Ot->Hasproto), Hasbody(Ot->Hasbody), IsStatic(Ot->IsStatic),
          Parent(Ot), IsFunctionPtr(Ot->IsFunctionPtr), TypeParams(Ot->TypeParams) {
...
TaintedFunctionVariableConstraint::TaintedFunctionVariableConstraint(DeclaratorDecl *D,
                                                       ProgramInfo &I,
                                                       const ASTContext &C)
        : TaintedFunctionVariableConstraint(
        D->getType(), D,
        D->getDeclName().isIdentifier() ? std::string(D->getName()) : "", I,
        C, D->getTypeSourceInfo()) {}
...
TaintedFunctionVariableConstraint::TaintedFunctionVariableConstraint(TypedefDecl *D,
                                                       ProgramInfo &I,
                                                       const ASTContext &C)
        : TaintedFunctionVariableConstraint(D->getUnderlyingType(), nullptr,
                                     D->getNameAsString(), I, C,
                                     D->getTypeSourceInfo()) {}
...
TaintedFunctionVariableConstraint::TaintedFunctionVariableConstraint(
        const QualType QT, DeclaratorDecl *D, std::string N, ProgramInfo &I,
        const ASTContext &Ctx, TypeSourceInfo *TSInfo)
        : TaintedConstraintVariable(TaintedConstraintVariable::FunctionVariable, QT, N),
          Parent(nullptr) {
  const Type *Ty = QT.getTypePtr();
  QualType RT, RTIType;
  Hasproto = false;
  ...
  if (Ty->isFunctionPointerType()) {
    ...
    for (unsigned J = 0; J < FT->getNumParams(); J++) {
      ...
      auto ParamVar =
              TFVComponentVariable(QT, ITypeT, ParmVD, PName, I, Ctx,
                                  &N, true, ParamHasItype, ParamIsTainted);
      ParamVars.push_back(ParamVar);
    }
  ...
  // ConstraintVariable for the return.
  ReturnVar = TFVComponentVariable(RT, RTIType, D, RETVAR, I, Ctx,
                                  &N, true, ReturnHasItype, ReturnIsTaintedType);
  ...
  auto &CS = I.getConstraints();
  for(int idx : Voids) {
    TFVComponentVariable *FVCV = idx == -1 ? &ReturnVar : &ParamVars[idx];
    auto Ext = FVCV->ExternalConstraint;
    if (ConvertFunc && !Ext->isOriginallyTainted() && !Ext->srcHasItype() &&
        Ext->getCvars().size() == 1) {
      FVCV->setGenericIndex(0);
      DidConvert = true;
    } else {
      if (!Ext->isOriginallyTainted()) {
        Ext->constrainToWild(CS, ReasonLoc(VOID_TYPE_REASON, PSL));
      }
    }
  }
...
bool TaintedFunctionVariableConstraint::anyChanges(const TaintedEnvironmentMap &E) const {
  return ReturnVar.ExternalConstraint->anyChanges(E) ||
         llvm::any_of(ParamVars, [&E](TFVComponentVariable CV) {
             return CV.ExternalConstraint->anyChanges(E);
         });
}
...
bool TaintedFunctionVariableConstraint::isSolutionTainted(
        const TaintedEnvironmentMap &E) const {
  return ReturnVar.ExternalConstraint->isSolutionTainted(E) ||
         llvm::any_of(ParamVars, [&E](TFVComponentVariable CV) {
             return CV.ExternalConstraint->isSolutionTainted(E);
         });
}
...
bool TaintedFunctionVariableConstraint::isSolutionFullyTainted(
        const TaintedEnvironmentMap &E) const {
  return ReturnVar.ExternalConstraint->isSolutionTainted(E) &&
         llvm::all_of(ParamVars, [&E](TFVComponentVariable CV) {
             return CV.ExternalConstraint->isSolutionTainted(E);
         });
}
...
bool TaintedFunctionVariableConstraint::hasTArr(const TaintedEnvironmentMap &E,
                                        int AIdx) const {
  return ReturnVar.ExternalConstraint->hasTArr(E, AIdx);
}

bool TaintedFunctionVariableConstraint::hasTNtArr(const TaintedEnvironmentMap &E,
                                          int AIdx) const {
  return ReturnVar.ExternalConstraint->hasTNtArr(E, AIdx);
}
...
TFVConstraint *TaintedFunctionVariableConstraint::getCopy(ReasonLoc &Rsn,
                                                  Constraints &CS) {
  TaintedFunctionVariableConstraint *Copy = new TaintedFunctionVariableConstraint(this);
  Copy->ReturnVar = TFVComponentVariable(&Copy->ReturnVar, Rsn, CS);
  // Make copy of ParameterCVs too.
  std::vector<TFVComponentVariable> FreshParams;
  for (auto &ParmPv : Copy->ParamVars)
    FreshParams.push_back(TFVComponentVariable(&ParmPv, Rsn, CS));
  Copy->ParamVars = FreshParams;
  return Copy;
}
...
void TPVConstraint::equateArgumentConstraints(ProgramInfo &Info, ReasonLoc &Rsn) {
  ...
  HasEqArgumentConstraints = true;
  TaintedconstrainConsVarGeq(this, this->ArgumentConstraints, Info.getConstraints(),
                      Rsn, Same_to_Same, true, &Info);
...
void TaintedFunctionVariableConstraint::equateFVConstraintVars(
        TaintedConstraintVariable *CV, ProgramInfo &Info, ReasonLoc &Rsn) const {
  if (TFVConstraint *FVCons = dyn_cast<TFVConstraint>(CV)) {
    for (auto &PCon : FVCons->ParamVars)
      PCon.InternalConstraint->equateArgumentConstraints(Info, Rsn);
    FVCons->ReturnVar.InternalConstraint->equateArgumentConstraints(Info, Rsn);
  }
...
void TaintedFunctionVariableConstraint::equateArgumentConstraints(ProgramInfo &Info,
                                                           ReasonLoc &Rsn) {
  if (HasEqArgumentConstraints) {
    return;
  }
  ...
  // Is this not a function pointer?
  if (!IsFunctionPtr) {
    TFVConstraint *DefnCons = nullptr;

    // Get appropriate constraints based on whether the function is static or
    // not.
    if (IsStatic) {
      DefnCons = reinterpret_cast<TFVConstraint *>(Info.getStaticFuncConstraint(Name, FileName));
    } else {
      DefnCons = reinterpret_cast<TFVConstraint *>(Info.getExtFuncDefnConstraint(Name));
    }
    assert(DefnCons != nullptr);
...
void TaintedPointerVariableConstraint::constrainToWild(Constraints &CS,
                                                const ReasonLoc &Rsn) const {
  // Find the first VarAtom. All atoms before this are ConstAtoms, so
  // constraining them isn't useful;
  VarAtom *FirstVA = nullptr;
...
void TaintedPointerVariableConstraint::constrainIdxTo(Constraints &CS, ConstAtom *C,
                                               unsigned int Idx,
                                               const ReasonLoc &Rsn,
                                               bool DoLB, bool Soft) {
  assert(C == CS.getPtr() || C == CS.getArr() || C == CS.getNTArr());
...
void TaintedPointerVariableConstraint::constrainOuterTo(Constraints &CS, ConstAtom *C,
                                                 const ReasonLoc &Rsn,
                                                 bool DoLB, bool Soft) {
  constrainIdxTo(CS, C, 0, Rsn, DoLB, Soft);
}
...
bool TaintedPointerVariableConstraint::anyArgumentIsWild(const TaintedEnvironmentMap &E) {
  return llvm::any_of(ArgumentConstraints, [&E](TaintedConstraintVariable *CV) {
      return !CV->isSolutionTainted(E);
  });
}
...
bool TaintedPointerVariableConstraint::isSolutionTainted(
        const TaintedEnvironmentMap &E) const {
  return (FV && FV->isSolutionTainted(E)) ||
         llvm::any_of(Vars, [this, &E](Atom *A) {
             return !isa<WildAtom>(getSolution(A, E));
         });
}

bool TaintedPointerVariableConstraint::isSolutionFullyTainted(
        const TaintedEnvironmentMap &E) const {
  return (!FV || FV->isSolutionTainted(E)) &&
         llvm::all_of(Vars, [this, &E](Atom *A) {
             return !isa<WildAtom>(getSolution(A, E));
...
bool TaintedPointerVariableConstraint::anyChanges(const TaintedEnvironmentMap &E) const {
  // If it was not checked in the input, then it has changed if it now has a
  // checked type.
  bool PtrChanged = false;
...
TPVConstraint *TaintedPointerVariableConstraint::getCopy(ReasonLoc &Rsn,
  ...
  while (VAIt != Copy->Vars.end() && CAIt != Copy->SrcVars.end()) {
    if (auto *CA = dyn_cast<ConstAtom>(*VAIt)) {
      ...
      VarAtom *FreshVA = CS.getFreshTaintedVar(VA->getName(), VA->getVarKind());
      //TODO: How do you handle for tainted pointers
      FreshVars.push_back(FreshVA);
      if (!isa<WildAtom>(*CAIt)){
        CS.addTaintedConstraint(CS.createTaintedGeq(*CAIt, FreshVA, Rsn, false));
      }
    }
    ++VAIt;
    ++CAIt;
...
const ConstAtom *
TaintedPointerVariableConstraint::getSolution(const Atom *A,
                                       const TaintedEnvironmentMap &E) const {
  const ConstAtom *CS = nullptr;
  if (const ConstAtom *CA = dyn_cast<ConstAtom>(A)) {
    CS = CA;
...
bool TaintedPointerVariableConstraint::hasTArr(const TaintedEnvironmentMap &E,
                                       int AIdx) const {
  int VarIdx = 0;
  for (const auto &C : Vars) {
    const ConstAtom *CS = getSolution(C, E);
...
bool TaintedPointerVariableConstraint::isNtConstantArr(const TaintedEnvironmentMap &E) const {
  // First check that this pointer is a constant sized array. This function is
  // only looking for constant sized arrays, so other array pointers should
  // return false.
  if (isConstantArr()) {
...
bool TaintedPointerVariableConstraint::isConstantArr() const {
  return ArrSizes.find(0) != ArrSizes.end() &&
         ArrSizes.at(0).first == O_SizedArray;
}
...
unsigned long TaintedPointerVariableConstraint::getConstantArrSize() const {
  assert("Pointer must be a constant array to get size." && isConstantArr());
  return ArrSizes.at(0).second;
}
...
bool TaintedPointerVariableConstraint::isTopAtomUnsizedArr() const {
  if (ArrSizes.find(0) != ArrSizes.end()) {
    return ArrSizes.at(0).first != O_SizedArray;
  }
  return true;
...
bool TaintedPointerVariableConstraint::hasSomeSizedArr() const {
  for (auto &AS : ArrSizes) {
    if (AS.second.first == O_SizedArray || AS.second.second == O_UnSizedArray) {
      return true;
    }
...
bool TaintedPointerVariableConstraint::solutionEqualTo(Constraints &CS,
                                                const TaintedConstraintVariable *CV,
                                                bool ComparePtyp) const {
  bool Ret = false;
  if (CV != nullptr) {
    if (const auto *PV = dyn_cast<TPVConstraint>(CV)) {
      auto &OthCVars = PV->Vars;
      if (isGeneric() || PV->isGeneric() ||
          Vars.size() == OthCVars.size()) {
        Ret = true;
        ...
        if (Ret) {
          TFVConstraint *OtherFV = PV->getFV();
          if (FV != nullptr && OtherFV != nullptr) {
            Ret = FV->solutionEqualTo(CS, OtherFV, ComparePtyp);
          } else if (FV != nullptr || OtherFV != nullptr) {
            // One of them has FV null.
...
void TaintedFunctionVariableConstraint::print(raw_ostream &O) const {
  O << "( ";
  ReturnVar.InternalConstraint->print(O);
  O << ", ";
  ReturnVar.ExternalConstraint->print(O);
...
void TaintedFunctionVariableConstraint::dumpJson(raw_ostream &O) const {
  O << "{\"FunctionVar\":{\"ReturnVar\":[";
  bool AddComma = false;
  ReturnVar.InternalConstraint->dumpJson(O);
  O << ", ";
...
bool TaintedFunctionVariableConstraint::srcHasItype() const {
  return ReturnVar.ExternalConstraint->srcHasItype();
}

bool TaintedFunctionVariableConstraint::srcHasBounds() const {
  return ReturnVar.ExternalConstraint->srcHasBounds();
}

bool TaintedFunctionVariableConstraint::solutionEqualTo(Constraints &CS,
                                                 const TaintedConstraintVariable *CV,
                                                 bool ComparePtyp) const {
  bool Ret = false;
  if (CV != nullptr) {
    if (const auto *OtherFV = dyn_cast<TFVConstraint>(CV)) {
      Ret = (numParams() == OtherFV->numParams()) &&
            ReturnVar.solutionEqualTo(CS, OtherFV->getCombineReturn(),
                                      ComparePtyp);
      for (unsigned I = 0; I < numParams(); I++) {
    ...
    } else if (const auto *OtherPV = dyn_cast<TPVConstraint>(CV)) {
      // When comparing to a pointer variable, it might be that the pointer is a
      // function pointer. This is handled by PVConstraint::solutionEqualTo.
      return OtherPV->solutionEqualTo(CS, this, ComparePtyp);
    }
...
std::string
TaintedFunctionVariableConstraint::mkString(Constraints &CS,
                                     const MkStringOpts &Opts) const {
  UNPACK_OPTS(EmitName, ForItype, EmitPointee, UnmaskTypedef, UseName,
              ForItypeBase);
  if (UseName.empty())
...
// Action depends on the kind of constraint (checked, ptyp),
//   which is inferred from the atom type
static void createTaintedAtomGeq(Constraints &CS, Atom *L, Atom *R,
                          const ReasonLoc &Rsn, ConsAction CAct, bool DoEqType) {
  ConstAtom *CAL, *CAR;
  VarAtom *VAL, *VAR;
  ConstAtom *Wild = CS.getWild();
  ...
  if (CAR != nullptr && CAL != nullptr) {
    ...
    switch (CAct) {
      case Same_to_Same:
        // Equality for checked.
        CS.addTaintedConstraint(CS.createTaintedGeq(L, R, Rsn, true));
            CS.addTaintedConstraint(CS.createTaintedGeq(R, L, Rsn, true));
            // Not for ptyp.
            CS.addTaintedConstraint(CS.createTaintedGeq(L, R, Rsn, false));
            // Unless indicated.
            if (DoEqType)
              CS.addTaintedConstraint(CS.createTaintedGeq(R, L, Rsn, false));
            break;
      case Safe_to_Wild:
        CS.addTaintedConstraint(CS.createTaintedGeq(L, R, Rsn, true));
            CS.addTaintedConstraint(CS.createTaintedGeq(L, R, Rsn, false));
...
void TaintedconstrainConsVarGeq(TaintedConstraintVariable *LHS, TaintedConstraintVariable *RHS,
  ...
  if (RHS->getKind() == LHS->getKind()) {
    if (TFVConstraint *FCLHS = dyn_cast<TFVConstraint>(LHS)) {
      if (TFVConstraint *FCRHS = dyn_cast<TFVConstraint>(RHS)) {
        ...
        // Constrain the return values covariantly.
        // FIXME: Make neg(CA) here? Function pointers equated.
        TaintedconstrainConsVarGeq(FCLHS->getExternalReturn(),
                            FCRHS->getExternalReturn(), CS, Reason, Same_to_Same,
                            DoEqType, Info, HandleBoundsKey);
        TaintedconstrainConsVarGeq(FCLHS->getInternalReturn(),
                            FCRHS->getInternalReturn(), CS, Reason, Same_to_Same,
                            DoEqType, Info, HandleBoundsKey);
        ...
        if (FCLHS->numParams() == FCRHS->numParams()) {
          for (unsigned I = 0; I < FCLHS->numParams(); I++) {
            TaintedConstraintVariable *LHSV = FCLHS->getExternalParam(I);
            TaintedConstraintVariable *RHSV = FCRHS->getExternalParam(I);
            // FIXME: Make neg(CA) here? Now: Function pointers equated.
            TaintedconstrainConsVarGeq(RHSV, LHSV, CS, Reason, Same_to_Same, DoEqType,
                                Info, HandleBoundsKey);

            TaintedConstraintVariable *LParam = FCLHS->getInternalParam(I);
            TaintedConstraintVariable *RParam = FCRHS->getInternalParam(I);
            TaintedconstrainConsVarGeq(RParam, LParam, CS, Reason, Same_to_Same, DoEqType,
                                Info, HandleBoundsKey);
          }
        } else {
          // Constrain both to be top.
    ...
    } else if (TPVConstraint *PCLHS = dyn_cast<TPVConstraint>(LHS)) {
      if (TPVConstraint *PCRHS = dyn_cast<TPVConstraint>(RHS)) {
        ...
        if (CLHS.size() != 0) {
          if (CLHS.size() == CRHS.size() ||
            ...
            for (unsigned N = 0; N < Min; N++) {
              ...
              // Get outermost pointer first, using current ConsAction.
              if (N == 0)
                createTaintedAtomGeq(CS, IAtom, JAtom, Reason, CA, DoEqType);
              else {
                // Now constrain the inner ones as equal.
                createTaintedAtomGeq(CS, IAtom, JAtom, Reason, CA, true);
              }
            }
            // Unequal sizes means casting from (say) T** to T*; not safe.
            // unless assigning to a generic type.
          ...
          // Equate the corresponding FunctionConstraint.
          TaintedconstrainConsVarGeq(PCLHS->getFV(), PCRHS->getFV(), CS, Reason, CA,
                              DoEqType, Info, HandleBoundsKey);
        }
      } else
        llvm_unreachable("impossible");
    ...
    auto Reason = ReasonLoc(
            "Assigning from a function variable to a pointer variable",
            Rsn.Location);
    TPVConstraint *PCLHS = dyn_cast<TPVConstraint>(LHS);
    TFVConstraint *FCRHS = dyn_cast<TFVConstraint>(RHS);
    if (PCLHS && FCRHS) {
      if (TFVConstraint *FCLHS = PCLHS->getFV()) {
        TaintedconstrainConsVarGeq(FCLHS, FCRHS, CS, Reason, CA, DoEqType, Info,
                            HandleBoundsKey);
      } else {
        auto WildReason = ReasonLoc("Function:" + FCRHS->getName() +
                                    " assigned to non-function pointer.",
...
void TaintedconstrainConsVarGeq(TaintedConstraintVariable *LHS, const TVarSet &RHS,
                         Constraints &CS, const ReasonLoc &Rsn,
                         ConsAction CA, bool DoEqType, ProgramInfo *Info,
                         bool HandleBoundsKey) {
  for (const auto &J : RHS)
    TaintedconstrainConsVarGeq(LHS, J, CS, Rsn, CA, DoEqType, Info, HandleBoundsKey);
}

// Given an RHS and a LHS, constrain them to be equal.
void TaintedconstrainConsVarGeq(const TVarSet &LHS, const TVarSet &RHS,
                         Constraints &CS, const ReasonLoc &Rsn,
                         ConsAction CA, bool DoEqType, ProgramInfo *Info,
                         bool HandleBoundsKey) {
  for (const auto &I : LHS)
    TaintedconstrainConsVarGeq(I, RHS, CS, Rsn, CA, DoEqType, Info, HandleBoundsKey);
}
...
bool isAValidPVConstraint(const TaintedConstraintVariable *C) {
  if (const auto *PV = dyn_cast_or_null<TPVConstraint>(C))
    return !PV->getCvars().empty();
  return false;
}
...
void TaintedPointerVariableConstraint::mergeDeclaration(TaintedConstraintVariable *FromCV,
                                                 ProgramInfo &Info,
                                                 std::string &ReasonFailed) {
  TPVConstraint *From = dyn_cast<TPVConstraint>(FromCV);
  std::vector<Atom *> NewVAtoms;
  std::vector<ConstAtom *> NewSrcAtoms;
  CAtoms CFrom = From->getCvars();
  if (CFrom.size() != Vars.size()) {
...
void TaintedPointerVariableConstraint::equateWithItype(
        ProgramInfo &I, const ReasonLoc &ReasonUnchangeable) {
  Constraints &CS = I.getConstraints();
  assert(SrcVars.size() == Vars.size());
  for (unsigned VarIdx = 0; VarIdx < Vars.size(); VarIdx++) {
    ConstAtom *CA = SrcVars[VarIdx];
    if (isa<WildAtom>(CA))
      CS.addTaintedConstraint(CS.createTaintedGeq(Vars[VarIdx], CA,ReasonUnchangeable,true));
    else
      Vars[VarIdx] = SrcVars[VarIdx];
  }
  if (FV) {
...
void TaintedFunctionVariableConstraint::mergeDeclaration(TaintedConstraintVariable *FromCV,
                                                  ProgramInfo &I,
                                                  std::string &ReasonFailed) {
  // `this`: is the declaration the tool saw first.
  // `FromCV`: is the declaration seen second

  TFVConstraint *From = dyn_cast<TFVConstraint>(FromCV);
  assert(From != nullptr);
  assert("this should have more params" &&
         this->numParams() >= From->numParams());
...
bool TaintedFunctionVariableConstraint::isOriginallyTainted() const {
  return ReturnVar.ExternalConstraint->isOriginallyTainted();
}
...
void TaintedFunctionVariableConstraint::equateWithItype(
        ProgramInfo &I, const ReasonLoc &ReasonUnchangeable) {
  ReturnVar.equateWithItype(I, ReasonUnchangeable);
  for (auto Param : ParamVars)
    Param.equateWithItype(I, ReasonUnchangeable);
...
void TFVComponentVariable::mergeDeclaration(TFVComponentVariable *From,
                                           ProgramInfo &I,
                                           std::string &ReasonFailed) {
  if (InternalConstraint == ExternalConstraint &&
      From->InternalConstraint != From->ExternalConstraint) {
...
std::string TFVComponentVariable::mkString(Constraints &CS, bool EmitName,
                                          bool ForItypeBase) const {
  return mkTypeStr(CS, EmitName, "", ForItypeBase) +
         mkItypeStr(CS, ForItypeBase);
}
...
std::string TFVComponentVariable::mkTypeStr(Constraints &CS, bool EmitName,
  ...
  // variable. Because the call is passed ForItypeBase, it will emit an
  // unchecked type instead of the solved type.
  if (ForItypeBase || hasTaintedSolution(CS) || (EmitName && !UseName.empty())) {
    Ret = ExternalConstraint->mkString(
            CS, MKSTRING_OPTS(EmitName = EmitName, UseName = UseName,
                              ForItypeBase = ForItypeBase));
  } else {
...
std::string
TFVComponentVariable::mkItypeStr(Constraints &CS, bool ForItypeBase) const {
  if (!ForItypeBase && hasItypeSolution(CS))
    return " : itype(" +
           ExternalConstraint->mkString(
                   CS, MKSTRING_OPTS(EmitName = false, ForItype = true)) +
...
bool TFVComponentVariable::hasTaintedSolution(Constraints &CS) const {
  ...
  // constraint variables solve to the same type, then they are both checked and
  // we can use a _Ptr type.
  return ExternalConstraint->isSolutionTainted(CS.getVariables()) &&
         ((ExternalConstraint->srcHasItype() &&
           InternalConstraint->isSolutionTainted(CS.getVariables())) ||
          ExternalConstraint->anyChanges(CS.getVariables())) &&
         InternalConstraint->solutionEqualTo(CS, ExternalConstraint);
}
...
bool TFVComponentVariable::hasItypeSolution(Constraints &CS) const {
  // As in hasCheckedSolution, we want the variable to be advertised as checked
  // if the external variable is checked. If the external is checked, but the
  // internal is not equal to the external, then the internal is unchecked, so
  // have to use an itype instead of a _Ptr type.
...
TFVComponentVariable::TFVComponentVariable(const QualType &QT,
                                         ...
                                         bool IsTaintedFunctionPointer) {
  ExternalConstraint =
          new TPVConstraint(QT, D, N, I, C, InFunc, -1, PotentialGeneric, IsTaintedFunctionPointer,
                           nullptr, ITypeT);
  if (!HasItype && QT->isVoidPointerType()) {
    InternalConstraint = ExternalConstraint;
  } else {
...
void TFVComponentVariable::equateWithItype(ProgramInfo &I,
                                          const ReasonLoc &ReasonUnchangeable) const {
  Constraints &CS = I.getConstraints();
  const ReasonLoc &ReasonUnchangeable2 =
          (ReasonUnchangeable.isDefault() && ExternalConstraint->isGeneric())
...
void TFVComponentVariable::linkInternalExternal(ProgramInfo &I,
                                               bool EquateChecked) const {
  Constraints &CS = I.getConstraints();
  auto LinkReason = ReasonLoc("Function Internal/External Link", PersistentSourceLoc());
  for (unsigned J = 0; J < InternalConstraint->getCvars().size(); J++) {
    ...
    if (isa<VarAtom>(InternalA) || isa<VarAtom>(ExternalA)) {
      // Equate pointer types for internal and external parameter constraint
      // variables.
      CS.addTaintedConstraint(CS.createTaintedGeq(InternalA, ExternalA, LinkReason, false));
      CS.addTaintedConstraint(CS.createTaintedGeq(ExternalA, InternalA, LinkReason, false));
      ...
      if (!isa<ConstAtom>(ExternalA)) {
        ...
        // variable solves to WILD only when there is an unsafe use that
        // cannot be resolved by inserting casts.
        CS.addTaintedConstraint(CS.createTaintedGeq(InternalA, ExternalA,
                                      LinkReason, true));
        ...
        if (_3COpts.DisableFunctionEdges || _3COpts.DisableRDs ||
            EquateChecked || (ExternalConstraint->getName() == RETVAR && J > 0))
          CS.addTaintedConstraint(CS.createTaintedGeq(ExternalA, InternalA,
                                        LinkReason, true));
      }
    }
  }
  if (TFVConstraint *ExtFV = ExternalConstraint->getFV()) {
    TFVConstraint *IntFV = InternalConstraint->getFV();
    assert(IntFV != nullptr);
    TaintedconstrainConsVarGeq(ExtFV, IntFV, CS, LinkReason, Same_to_Same, true, &I);
  }
}
...
bool TFVComponentVariable::solutionEqualTo(Constraints &CS,
                                          const TFVComponentVariable *Other,
                                          bool ComparePtyp) const {
  bool InternalEq =
          getInternal()->solutionEqualTo(CS, Other->getInternal(), ComparePtyp);
  bool ExternalEq =
...
TFVComponentVariable::TFVComponentVariable(TFVComponentVariable *Ot,
                                         ReasonLoc &Rsn,
                                         Constraints &CS) {
  InternalConstraint = Ot->InternalConstraint->getCopy(Rsn,CS);
  ExternalConstraint = Ot->ExternalConstraint->getCopy(Rsn,CS);
